...
class DVRouter(Router):
    ...
    def integrate(self,link,adv):
        for dest, cost in adv:
            total_cost = cost + link.cost
            if not dest in self.spcost or total_cost < self.spcost[dest]:
                if total_cost < self.INFINITY:
                    self.routes[dest] = link
                    self.spcost[dest] = total_cost
            elif self.routes[dest] == link and total_cost != self.spcost[dest]:
                self.spcost[dest] = total_cost
