...
class LSRouter(Router):
    ...
    def make_ls_advertisement(self):
        ...
        return [(self.neighbors[link][1], self.neighbors[link][2]) for link in self.neighbors]
    ...
    def run_dijkstra(self, nodes):
        X = nodes
        X.remove(self.address)
        S = [self.address]
        
        # while X is not empty
        while X:
            u = S[-1] # last added node
            for neighbor, linkcost in self.LSA.get(u)[1:]:
                if self.spcost[neighbor] > self.spcost[u] + linkcost:
                    self.spcost[neighbor] = self.spcost[u] + linkcost
                    if u == self.address:
                        self.routes[neighbor] = self.getlink(neighbor)
                    else:
                        self.routes[neighbor] = self.routes[u]
            spcost_X = {k: self.spcost[k] for k in X} # spcost for nodes in X
            min_spcost_node_X = min(spcost_X, key=spcost_X.get)
            X.remove(min_spcost_node_X)
            S.append(min_spcost_node_X)
