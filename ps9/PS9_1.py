...
class ReliableSenderNode(Router):
    ...
    def reliable_send(self, time):
        if self.p is not None:
            if time - self.p.start >= self.timeout:
                self.p = self.send_pkt(time, self.current_seqnum, time, 'black')
        else:
            self.p = self.send_pkt(time, self.current_seqnum, time, 'gray')

    # An ACK just arrived; process it.  Remember to call calc_timeout with the
    # appropriate information.
    def process_ack(self, time, acknum, timestamp):
        if acknum == self.current_seqnum:
            self.p = None
