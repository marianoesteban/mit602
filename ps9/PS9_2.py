...
class ReliableSenderNode(Router):
    ...
    def reliable_send(self, time):
        for p in self.unacked_pkts:
            if time - p.start > self.timeout:
                p.start = time
                self.send_pkt(time, p.properties['seqnum'], time, 'black')
                return
        
        if len(self.unacked_pkts) < self.window:
            self.max_seq += 1
            self.unacked_pkts.append(self.send_pkt(time, self.max_seq, time, 'gray'))
...
class ReliableReceiverNode(Router):
    ...
    def reliable_recv(self, sender, time, seqnum, pkt_timestamp):
        ...
        if seqnum > self.app_seqnum and seqnum not in self.rcvbuf:
            bisect.insort(self.rcvbuf, seqnum) # insert seqnum in sorted order
        
        # deliver contiguous packets to the application
        while seqnum == self.app_seqnum + 1:
            self.app_receive(seqnum, time)
            self.rcvbuf.remove(seqnum)
            if len(self.rcvbuf) == 0:
                break
            seqnum = self.rcvbuf[0]
